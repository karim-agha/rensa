...
impl ApiService {
  pub fn new(
    ...
    let svc = Router::new()
      ...
      .route("/transactions", post(serve_send_transactions))
...
async fn serve_send_transactions(
  ...
  for transaction in transactions.into_iter() {
    ...
    if let Err(e) = transaction.verify_limits(&state.genesis.limits) {
      outputs.push((transaction.hash().to_b58(), e.to_string()));
      continue;
    }
    ...
    // filter out transaction replays at the RPC level, and allow
    // a batch of transactions from the same payer with monotonically
    // increasing nonce.
    let expected_nonce = match nonces.entry(transaction.payer) {
    ...
    if let Err(e) = state.sender.send(transaction) {
      ...
      outputs.push((hash.to_b58(), "ok".to_owned()));
    }
  }

  // return a list of {transaction_hash: result} json object
...
///
/// KNOWN BEHAVIOR: Account deletions will not be reflected in the
/// confirmed commitment, only in finalized.
